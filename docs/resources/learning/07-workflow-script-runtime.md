...
However, [ADR 0002](../../areas/adr/0002-use-acorn-and-node-vm-for-first-workflow-runtime.md) explicitly says `node:vm` is not a complete security boundary. Treat this as an execution kernel for the first implementation, not a finished security story.
