...
import type { SavedWorkflowEntry } from "../workflow-library.js";
...
export function registerWorkflowExtension(
	pi: ExtensionAPI,
	deps: WorkflowExtensionDeps = createDefaultWorkflowExtensionDeps(),
): void {
	const {
		manager,
		workflowTool,
	...
	pi.registerMessageRenderer("workflow-completion", (message, _options, theme) => {
		const details = message.details as { jobId?: number; runId?: string; name?: string; status?: string } | undefined;
	...
	pi.registerCommand("workflows", {
		...
		handler: async (_args, ctx) => {
			...
			await ctx.ui.custom<void>((tui, theme, _kb, done) =>
				createBrowser(manager, tui, theme, done, {
					save: (job) => {
					...
					rerun: (job) => {
						...
						ctx.ui.notify(`Reran workflow #${job.id} as #${rerun.id}`, "info");
					},
					resume: (job) => {
	...
	pi.registerCommand("workflow-save", {
		...
		handler: async (args, ctx) => {
			...
			ctx.ui.notify(`Saved workflow #${id} globally as /${entry.name} (${entry.path})`, "info");
		},
	});
	...
	pi.registerCommand("workflow-list", {
		...
		handler: async (_args, ctx) => {
			...
			if (entries.length === 0) {
				ctx.ui.notify("No saved workflows. Save one with /workflow-save <job-id> [command-name].", "info");
				return;
			}
			ctx.ui.notify(
				entries.map((entry) => `/${entry.name} — ${entry.description} (${entry.path})`).join("\n"),
				"info",
			);
		},
	});
	...
	pi.registerCommand("workflow-delete", {
		description: "Delete a globally saved workflow command: /workflow-delete <command-name>",
		handler: async (args, ctx) => {
			...
			if (ctx.hasUI) {
				const ok = await ctx.ui.confirm("Delete saved workflow?", `Delete /${entry.name} from ${entry.path}?`);
				if (!ok) return;
			}
	...
	pi.registerCommand("workflow-edit", {
		description: "Edit a globally saved workflow command script: /workflow-edit <command-name>",
		handler: async (args, ctx) => {
			const name = args.trim().replace(/^\//, "");
			if (!name) {
			...
			const edited = await ctx.ui.editor(`Edit saved workflow /${entry.name}`, entry.script);
	...
	pi.registerCommand("workflow-refresh", {
		description: "Reload saved workflow files and register any new slash commands",
		handler: async (_args, ctx) => {
			...
			ctx.ui.notify(`Refreshed saved workflows (${added} new command${added === 1 ? "" : "s"}).`, "info");
		},
	});

	async function saveWorkflowJobFromDashboard(job: WorkflowJob, ctx: ExtensionContext): Promise<void> {
		const name = await ctx.ui.input("Save workflow as slash command", job.name);
		if (name === undefined) return;
		try {
			const entry = globalWorkflowLibrary.save(job.script, name.trim() || job.name);
			registerSavedWorkflowCommand(entry);
			ctx.ui.notify(`Saved workflow #${job.id} globally as /${entry.name} (${entry.path})`, "info");
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
	...
	function registerSavedWorkflowCommand(entry: SavedWorkflowEntry): void {
		...
		pi.registerCommand(entry.name, {
			...
			handler: async (commandArgs, ctx) => {
				...
				if (!current) {
					ctx.ui.notify(`Saved workflow /${entry.name} no longer exists`, "error");
					return;
				}
	...
	pi.on("session_start", (_event, ctx) => {
		ctx.ui.setEditorComponent((tui, theme, keybindings) => new WorkflowTriggerEditor(tui, theme, keybindings));

		currentSessionRunIds.clear();
		for (const entry of ctx.sessionManager.getEntries()) {
			if (entry.type === "custom" && entry.customType === "workflow-notification-sent") {
				const data = entry.data as { runId?: string } | undefined;
				if (data?.runId) announcedRuns.add(data.runId);
			}
		}
		manager.attachStore(createWorkflowStore(ctx.cwd));
		registerSavedWorkflowCommands();
