...
Default to pipeline() for multi-stage work. A single-stage fan-out over a work-list (one operation per item, no second stage) is just parallel(); reach for pipeline() only when each item flows through two or more stages. Use a parallel() barrier only when a stage genuinely needs all earlier results together, such as deduplication, global merge, zero-count early exit, or cross-item comparison. Filter null results defensively after parallel() or pipeline() stages that may fail. A single parallel() or pipeline() call accepts at most 4096 items.
