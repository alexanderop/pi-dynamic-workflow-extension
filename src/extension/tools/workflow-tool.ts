...
Available workflow globals: args, budget, agent(prompt, opts?), pipeline(items, ...stages), parallel(thunks), phase(title), and log(message). agent() spawns one subagent; without opts.schema it returns final text, and with opts.schema it returns the validated structured object. opts.schema must be a plain JSON object schema suitable for Pi tool parameters, such as { type: 'object', properties: ..., required: ... }. Other opts may include label, phase, thinkingLevel, isolation: 'worktree', agentType, and the compatibility field model. By default, select the desired Pi model before launching the workflow, do not set model hints by default, and use thinkingLevel to vary reasoning effort. The model field is ignored unless /workflows features enables experimental-model-routing; with experimental-model-routing enabled, exact model hints may route agents and invalid hints fall back to the current Pi model. pipeline() has no cross-item barrier; each item advances through stages as soon as it can. Stage callbacks receive (prevResult, originalItem, index); for the first stage, prevResult === originalItem. parallel() is a barrier over thunks and returns null for failed thunks.
