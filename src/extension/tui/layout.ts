...
/**
 * Width-aware text-layout helpers for the `/workflows` TUI.
 *
 * These render terminal output and therefore depend on Pi TUI width
 * measurement (`visibleWidth` / `truncateToWidth`). ADR 0010 keeps
 * `src/workflows/view/` TUI-agnostic, so the column-aware helpers live in the
 * extension TUI layer while the pure formatters stay in
 * `src/workflows/view/layout.ts`.
 */
